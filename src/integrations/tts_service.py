...
class TTSService:
            ...
            try:
                ...
                if device.startswith("cuda"):
                    ...
                    with torch.cuda.device(device):
                        ...
                        self._dia_model = self.gpu_manager.optimize_model_for_inference(
                            self._dia_model
                        )
                        ...
                        if vram_info:
                            ...
                            self.logger.info(
                                f"Dia model loaded to GPU, VRAM used: {used_gb:.1f}GB"
                            )
                else:
                    # CPU fallback
                    self.logger.info(
                        "Loading Dia model to CPU due to insufficient VRAM"
                    )
                    self._dia_model = Dia.from_pretrained("nari-labs/Dia-1.6B")
    ...
    def _generate_with_elevenlabs(
        ...
        try:
            ...
            voice_settings = self._get_elevenlabs_voice_settings(
                segment.emotion, segment.pacing
            )
    ...
    def _generate_with_dia(
        ...
        try:
            ...
            if self._dia_model is None:
                self.logger.warning(
                    "Dia model not initialized, cannot generate speech."
                )
                return None
                ...
                if device.type == "cuda":
                    with torch.cuda.device(device):
                        ...
                        if vram_before:
                            ...
                            if vram_after:
                                used_mb = (
                                    vram_after["used"] - vram_before["used"]
                                ) / 1024**2
                                self.logger.debug(
                                    f"TTS generation used {used_mb:.0f}MB VRAM"
                                )
                else:
                    # CPU generation
                    try:
    ...
    def _get_elevenlabs_voice_settings(
        self, emotion: EmotionType, pacing: PacingType
    ) -> VoiceSettings:
        """Map emotion and pacing to ElevenLabs voice settings"""
        # Base settings
        stability = 0.5
    ...
    def _enhance_text_for_dia(self, segment: NarrativeSegment) -> str:
        ...
        if segment.emotion == EmotionType.EXCITED:
            ...
            if (
                "amazing" in text.lower()
                or "incredible" in text.lower()
                or "crazy" in text.lower()
            ):
