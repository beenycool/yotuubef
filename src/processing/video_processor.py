...
class ResourceManager:
    ...
    def __init__(self):
        ...
        self.gpu_manager = GPUMemoryManager(
            max_vram_usage=0.85
        )  # Use 85% VRAM for maximum performance
...
class VideoDownloader:
    ...
    def download_audio(
        self, url: str, output_path: Path, audio_format: str = "mp3"
    ) -> bool:
        """
        ...
        try:
            ...
            try:
                ...
                if any(
                    ...
                    self.logger.warning(
                        f"Primary audio format failed for {url}, trying fallbacks"
                    )
                    ...
                    for i, strategy in enumerate(fallback_strategies):
                        try:
                            self.logger.info(
                                f"Trying audio fallback strategy {i + 1}/{len(fallback_strategies)}"
                            )
                            fallback_opts = ydl_opts.copy()
                            ...
                            self.logger.debug(
                                f"Audio fallback strategy {i + 1} failed: {fallback_error}"
                            )
                            continue
    ...
    def download_both(
        ...
        try:
            ...
            self.logger.info(
                f"Video download {'successful' if results['video'] else 'failed'}"
            )
        except Exception as e:
        ...
        try:
            ...
            self.logger.info(
                f"Audio download {'successful' if results['audio'] else 'failed'}"
            )
        except Exception as e:
        ...
        try:
            ...
            for file_path in downloaded_files:
                if file_path.suffix.lower() in video_extensions:
                    ...
                    self.logger.info(
                        f"Successfully extracted Reddit video with generic method: {final_path}"
                    )
                    return True
        ...
        try:
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                return {
                    ...
                    "formats": info.get(
                        "formats", []
                    ),  # Include available formats for debugging
                }
...
class VideoEffects:
    ...
    def apply_speed_effects(
        self, clip: VideoFileClip, speed_effects: List[Dict]
    ) -> VideoFileClip:
        ...
        try:
            ...
            for effect in speed_effects:
                ...
                speed_factor = max(
                    0.5, min(2.0, speed_factor)
                )  # Keep within reasonable bounds

                # Add normal segment before effect if needed
                if current_time < start_time:
                    normal_segment = MoviePyCompat.subclip(
                        clip, current_time, start_time
                    )
                ...
                if speed_factor != 1.0:
                    try:
                        effect_segment = effect_segment.with_fps(
                            effect_segment.fps * speed_factor
                        )
                    except Exception as e:
                    ...
                    if segments and speed_factor > 1.0:  # Only for speed-ups
                        ...
                        effect_segment = MoviePyCompat.crossfadein(
                            effect_segment, transition_duration
                        )
            ...
            if current_time < clip.duration:
                segments.append(
                    MoviePyCompat.subclip(clip, current_time, clip.duration)
                )
    ...
    def add_visual_cues(
        self, clip: VideoFileClip, visual_cues: List[VisualCue]
    ) -> VideoFileClip:
    ...
    def _create_zoom_highlight(
        ...
        try:
            ...
            overlay = MoviePyCompat.with_start(
                MoviePyCompat.with_duration(overlay, duration), timestamp
            )
    ...
    def apply_dynamic_pan_zoom(
        self, clip: VideoFileClip, focus_points: List[Dict[str, Any]]
    ) -> VideoFileClip:
        """
        ...
        if not focus_points or len(focus_points) < 2:
            self.logger.info(
                "Insufficient focus points for dynamic pan/zoom, applying subtle zoom instead"
            )
        ...
        try:
            ...
            sorted_points = sorted(
                focus_points, key=lambda x: x.get("timestamp_seconds", 0)
            )
            ...
            if len(sorted_points) > 3:
                transformed_clip = self._apply_motion_blur(
                    transformed_clip, motion_path
                )

            self.logger.info(
                f"Applied enhanced dynamic pan/zoom with {len(sorted_points)} focus points"
            )
    ...
    def _validate_focus_points(
        self, focus_points: List[Dict[str, Any]], duration: float
    ):
        ...
        for point in focus_points:
            ...
            if point_time < prev_time:
                raise ValueError(
                    f"Focus points not in chronological order at {point_time}"
                )
    ...
    def _calculate_motion_path(
        ...
        def motion_path_func(t):
            ...
            if current_seg:
                ...
                x = current_seg["start_x"] + current_seg["x_vel"] * (
                    t - current_seg["start_time"]
                )
                y = current_seg["start_y"] + current_seg["y_vel"] * (
                    t - current_seg["start_time"]
                )
    ...
    def _calculate_crop_window(
        self, w: int, h: int, focus_x: int, focus_y: int, crop_w: int, crop_h: int
    ):
    ...
    def _apply_motion_blur(self, clip: VideoFileClip, motion_path):
        ...
        if avg_vel > 0.01:
            ...
            return MoviePyCompat.apply_fx_effect(
                clip, vfx.motion_blur, blur_amount, 0.5
            )
        return clip
    ...
    def _create_focus_segment(
        ...
        try:
            ...
            focused_segment = MoviePyCompat.crop(
                segment, x1=crop_x1, y1=crop_y1, x2=crop_x2, y2=crop_y2
            )
            ...
            self.logger.debug(
                f"Created focus segment at ({x:.2f}, {y:.2f}): {description}"
            )
...
class AdvancedVideoEnhancer:
    ...
    def _apply_ffmpeg_filters(
        ...
        try:
            # module-level imports already available
            ...
            self.logger.warning(
                "FFmpeg not found - install FFmpeg for advanced filters"
            )
            return None
        except Exception as e:
    ...
    def apply_adaptive_color_grading(self, clip: VideoFileClip) -> VideoFileClip:
        ...
        try:
            ...
            sample_times = [
                clip.duration * i / 6 for i in range(1, 6)
            ]  # 5 samples across the video
            brightness_samples = []
            contrast_samples = []
            saturation_samples = []
            ...
            if brightness_samples:
                ...
                avg_contrast = (
                    np.mean(contrast_samples) / 127.5
                )  # Normalize to 0-2 range
                avg_saturation = np.mean(saturation_samples) / 255.0

                self.logger.info(
                    f"Video analysis - Brightness: {avg_brightness:.2f}, Contrast: {avg_contrast:.2f}, Saturation: {avg_saturation:.2f}"
                )
                ...
                if avg_brightness < 0.3:  # Dark video - make it pop
                    ...
                    if vfx and hasattr(vfx, "colorx"):
                        clip = MoviePyCompat.apply_fx_effect(
                            clip, vfx.colorx, factor=1.3
                        )
                    if vfx and hasattr(vfx, "lum_contrast"):
                        clip = MoviePyCompat.apply_fx_effect(
                            clip, vfx.lum_contrast, lum=0.15, contrast=0.4
                        )
                    self.logger.info(
                        "Applied dark video enhancement with aggressive pop"
                    )
                    ...
                    if vfx and hasattr(vfx, "colorx"):
                        clip = MoviePyCompat.apply_fx_effect(
                            clip, vfx.colorx, factor=0.85
                        )
                    if vfx and hasattr(vfx, "lum_contrast"):
                        clip = MoviePyCompat.apply_fx_effect(
                            clip, vfx.lum_contrast, lum=-0.1, contrast=0.35
                        )
                    self.logger.info(
                        "Applied bright video enhancement with contrast boost"
                    )
                    ...
                    if vfx and hasattr(vfx, "colorx"):
                        clip = MoviePyCompat.apply_fx_effect(
                            clip, vfx.colorx, factor=1.15
                        )
                    if vfx and hasattr(vfx, "lum_contrast"):
                        clip = MoviePyCompat.apply_fx_effect(
                            clip, vfx.lum_contrast, lum=0.05, contrast=0.3
                        )
                ...
                if avg_contrast < 0.8:  # Low contrast - enhance shadows and highlights
                    ...
                    def shadow_highlight_enhancement(get_frame, t):
                        ...
                        shadows = np.where(
                            frame_float < 0.3, frame_float * 1.3, frame_float
                        )
                    ...
                    clip = MoviePyCompat.apply_effect(
                        clip, shadow_highlight_enhancement
                    )
...
class TextOverlayProcessor:
    ...
    def add_hook_text(
        self, clip: VideoFileClip, analysis: VideoAnalysis
    ) -> VideoFileClip:
        ...
        try:
            ...
            hook_overlay = TextOverlay(
                ...
                duration=min(
                    3.0, clip.duration * 0.15
                ),  # Show for 3 seconds or 15% of video
                position="center",
                style="dramatic",
            )
    ...
    def _create_hook_text_clip(
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(
                    f"Skipping hook text animation due to resize issue: {e}"
                )
    ...
    def _create_text_clip(
        ...
        try:
            ...
            font_path = self.config.get_font_path(
                self.config.text_overlay.graphical_font
            )
            ...
            if text_length <= 10:
                size_ratio = self.config.text_overlay.font_size_ratio_profiles.get(
                    "short", 0.06
                )
            elif text_length <= 30:
                size_ratio = self.config.text_overlay.font_size_ratio_profiles.get(
            ...
            position = self._calculate_text_position(
                overlay.position, video_clip.size, text_clip.size
            )
            ...
            text_clip = self._apply_text_animation(
                text_clip, overlay.style, overlay.duration
            )
    ...
    def _calculate_text_position(
        self, position_type: str, video_size: tuple, text_size: tuple
    ) -> tuple:
        ...
        position_map = {
            ...
            "top": ((video_width - text_width) // 2, margin_y),
            "bottom": (
                (video_width - text_width) // 2,
                video_height - text_height - margin_y,
            ),
            "left": (margin_x, (video_height - text_height) // 2),
            "right": (
    ...
    def _apply_text_animation(
        self, text_clip: TextClip, style: str, duration: float
    ) -> TextClip:
        ...
        try:
            ...
            if style == "dramatic":
                ...
                text_clip = MoviePyCompat.resize(
                    text_clip, lambda t: 1 + 0.1 * (1 - abs(2 * t / duration - 1))
                )
                ...
                # Subtle pulse effect
                text_clip = MoviePyCompat.resize(
                    text_clip, lambda t: 1 + 0.05 * np.sin(t * 4)
                )
...
class AudioProcessor:
    ...
    def process_audio(
        ...
        try:
            ...
            music_clip = self._add_background_music_safe(
                background_music_path, video_clip.duration, narrative_segments
            )
            if music_clip:
            ...
            if sound_effects and base_composite:
                final_audio = self.add_sound_effects(
                    base_composite, sound_effects, video_clip.duration
                )
                return final_audio
            elif base_composite:
                ...
                self.logger.warning(
                    "No audio tracks available, returning original audio"
                )
    ...
    def _process_original_audio(
        ...
        try:
            if video_clip.audio and self.config.audio.original_audio_mix_volume > 0:
                try:
                    original_audio = video_clip.audio.with_volume_scaled(
    ...
    def _add_background_music_safe(
        self,
        ...
        if not (
            ...
            and self.config.audio.background_music_enabled
        ):
        ...
        try:
            ...
            self.logger.warning(
                f"Failed to add background music from {music_path}: {e}"
            )
            return None

    def _process_narrative_segments_safe(
        ...
        try:
            ...
            for result in tts_results:
                ...
                try:
                    ...
                    audio_clip = MoviePyCompat.with_start(
                        audio_clip, segment.time_seconds
                    )
                    ...
                    self.logger.debug(
                        f"Successfully processed TTS segment: '{segment.text[:50]}...'"
                    )
            ...
            self.logger.info(
                f"Successfully processed {len(successful_audio_clips)}/{len(segments)} TTS segments"
            )
    ...
    def _add_background_music(
        ...
        try:
            ...
            # Apply base volume
            try:
                music_clip = music_clip.with_volume_scaled(
                    self.config.audio.background_music_volume
                )
            except Exception as e:
    ...
    def _get_emotion_volume_factor(self, emotion: str) -> float:
        ...
        emotion_volumes = {"excited": 1.1, "dramatic": 1.2, "calm": 0.9, "neutral": 1.0}
    ...
    def add_sound_effects(
        ...
        try:
            ...
            for effect in sound_effects:
                try:
                    ...
                    if timestamp >= video_duration:
                        self.logger.debug(
                            f"Skipping sound effect '{effect_name}' - timestamp beyond video duration"
                        )
                        continue

                    # Use enhanced sound effects manager to find the best match
                    sound_file = self.sound_effects_manager.find_sound_effect(
                        effect_name, preferred_category=preferred_category
                    )

                    if sound_file:
                        # Validate the sound file before using
                        is_valid, validation_msg = (
                            self.sound_effects_manager.validate_sound_effect(sound_file)
                        )
                        if not is_valid:
                            self.logger.warning(
                                f"Invalid sound effect file '{sound_file}': {validation_msg}"
                            )
                            effects_failed += 1
                            continue
                        ...
                        try:
                            ...
                            self.logger.warning(
                                f"Error applying volume to sound effect: {e}"
                            )
                        ...
                        if timestamp + sound_clip.duration > video_duration:
                            ...
                            if (
                                max_duration > 0.1
                            ):  # Only add if at least 0.1 seconds remaining
                                sound_clip = MoviePyCompat.subclip(
                                    sound_clip, 0, max_duration
                                )
                            else:
                                self.logger.debug(
                                    f"Skipping sound effect '{effect_name}' - insufficient time remaining"
                                )
                                sound_clip.close()
                                continue
                        ...
                        if sound_clip.duration > fade_duration * 2:
                            try:
                                ...
                                sound_clip = sound_clip.with_fadein(
                                    fade_duration
                                ).with_fadeout(fade_duration)
                            except AttributeError:
                                try:
                                    # Try older MoviePy method names
                                    sound_clip = sound_clip.audio_fadein(
                                        fade_duration
                                    ).audio_fadeout(fade_duration)
                                except AttributeError:
                                    # Skip fade effects if not available
                                    self.logger.debug(
                                        "Fade effects not available for sound clips"
                                    )

                        audio_tracks.append(sound_clip)
                        effects_added += 1
                        self.logger.info(
                            f"Added sound effect '{effect_name}' at {timestamp:.1f}s (volume: {volume:.1f}, file: {sound_file.name})"
                        )
                        ...
                        self.logger.warning(
                            f"Sound effect file not found for: {effect_name}"
                        )
                        effects_failed += 1

                except Exception as e:
                    self.logger.warning(
                        f"Failed to add sound effect '{effect.get('effect_name', 'unknown')}': {e}"
                    )
                    effects_failed += 1
                    continue
            ...
            success_rate = (
                (effects_added / total_requested * 100) if total_requested > 0 else 0
            )
            self.logger.info(
                f"Sound effects summary: {effects_added}/{total_requested} added successfully ({success_rate:.1f}% success rate)"
            )

            if effects_failed > 0:
                cache_status = self.sound_effects_manager.get_cache_status()
                self.logger.info(
                    f"Available sound effects: {cache_status['total_effects']} across {cache_status['categories']} categories"
                )
    ...
    def _process_narrative_segments(
        self, segments: List[NarrativeSegment]
    ) -> List[AudioFileClip]:
...
class VideoProcessor:
    ...
    def process_video(
        ...
        with (
            ResourceManager() as resource_manager,
            TemporaryFileManager() as temp_manager,
        ):
            try:
                self.logger.info(f"Starting enhanced video processing: {video_path}")

                # Stage 1: Input Validation and Preparation
                video_clip = self._load_source_clip(
                    video_path, output_path, analysis, resource_manager
                )
                if not video_clip:
                    return result

                # Stage 2: Enhanced Audio Synthesis with TTS and Sound Effects
                composite_audio = self._synthesize_audio(
                    video_clip,
                    analysis,
                ...
                enhanced_visuals = self._compose_visuals(
                    video_clip, analysis, resource_manager
                )

                # Stage 4: Add Call-to-Action Elements for Maximum Engagement
                final_visuals = self._add_engagement_elements(
                    enhanced_visuals, analysis, resource_manager
                )
                ...
                final_clip = (
                    MoviePyCompat.with_audio(final_visuals, final_audio)
                    if final_audio
                    else final_visuals
                )
                video_success = self._render_video(
                    final_clip, output_path, temp_manager
                )
                ...
                if video_success:
                    ...
                    result["processing_stats"].update(
                        {
                            "tts_segments": len(analysis.narrative_script_segments)
                            if analysis.narrative_script_segments
                            else 0,
                            "visual_effects": len(analysis.visual_cues)
                    ...
                    result["success"] = True
                    self.logger.info(
                        f"Enhanced video processing completed: {output_path}"
                    )
                else:
                ...
                self.logger.error(
                    f"Critical error in video processing: {e}", exc_info=True
                )
                return result
            finally:
                self._cleanup()
    ...
    def _load_source_clip(
        ...
        try:
            ...
            self.logger.info(
                f"Successfully loaded source clip: {video_clip.duration:.2f}s"
            )
            return video_clip
    ...
    def _synthesize_audio(
        ...
        try:
            self.logger.info(
                "Starting enhanced audio synthesis with TTS and sound effects"
            )
            ...
            if analysis.narrative_script_segments:
                try:
                    ...
                    self.logger.warning(
                        f"TTS processing failed, continuing without narration: {e}"
                    )
            ...
            if background_music_path and background_music_path.exists():
                try:
                    ...
                    if music_clip:
                        ...
                        self.logger.info(
                            "Added background music with intelligent ducking"
                        )
                except Exception as e:
            ...
            if len(audio_elements) > 1:
                ...
                self.logger.info(
                    f"Created base composite audio with {len(audio_elements)} elements"
                )
            elif audio_elements:
            ...
            if base_composite and analysis.sound_effects:
                try:
                    ...
                    resource_manager.register_clip(final_audio)
                    self.logger.info(
                        f"Added {len(analysis.sound_effects)} sound effects"
                    )
                    return final_audio
                except Exception as e:
                ...
                self.logger.warning(
                    "No audio elements available, creating fallback background music"
                )
                ...
                try:
                    ...
                    if fallback_audio:
                        ...
                        silent_audio = AudioClip(
                            make_frame=lambda t: [0, 0], duration=video_duration
                        )
    ...
    def _create_fallback_audio(
        self, video_duration: float, resource_manager: ResourceManager
    ) -> Optional[AudioFileClip]:
        """
        ...
        try:
            ...
            try:
                ...
                self.logger.info(
                    f"Created fallback background music: {video_duration:.2f}s at {background_volume:.2f} volume"
                )
                return music_clip

            except Exception as e:
                self.logger.error(
                    f"Failed to process fallback music {selected_music}: {e}"
                )
                return None
    ...
    def _compose_visuals(
        ...
        try:
            self.logger.info(
                "Starting enhanced visual composition with focus points and effects"
            )
            ...
            if self.config.video.video_quality_profile == "high":
                try:
                    video_clip = self.advanced_enhancer.apply_enhancement_filters(
                        video_clip, enable_denoising=True, enable_sharpening=True
                    )
            ...
            try:
                video_clip = self.advanced_enhancer.apply_adaptive_color_grading(
                    video_clip
                )
            ...
            if analysis.key_focus_points:
                try:
                    ...
                    video_clip = self.effects.apply_dynamic_pan_zoom(
                        video_clip, focus_data
                    )
                    self.logger.info(
                        f"Applied dynamic pan/zoom for {len(analysis.key_focus_points)} focus points"
                    )
                except Exception as e:
            ...
            if analysis.speed_effects:
                try:
                    ...
                    video_clip = self.effects.apply_speed_effects(
                        video_clip, speed_data
                    )
                    self.logger.info(
                        f"Applied {len(analysis.speed_effects)} speed effects for drama"
                    )
                except Exception as e:
                    self.logger.warning(f"Speed effects failed: {e}")

            # Apply visual effects with graceful error handling
            video_clip = self._apply_visual_effects(
                video_clip, analysis, resource_manager
            )
            ...
            if analysis.text_overlays:
                try:
                    video_clip = self.text_processor.add_text_overlays(
                        video_clip, analysis.text_overlays
                    )
                    self.logger.info(
                        f"Added {len(analysis.text_overlays)} text overlays with animations"
                    )
                except Exception as e:
            ...
            if analysis.visual_cues:
                try:
                    video_clip = self.effects.add_visual_cues(
                        video_clip, analysis.visual_cues
                    )
    ...
    def _add_engagement_elements(
        ...
        try:
            ...
            try:
                enhanced_video = self.cta_processor.add_visual_ctas(
                    enhanced_video, analysis
                )
            ...
            try:
                enhanced_video = self.cta_processor.create_engagement_hooks(
                    enhanced_video, analysis
                )
    ...
    def _validate_inputs(
        self, video_path: Path, output_path: Path, analysis: VideoAnalysis
    ) -> bool:
        """Validate input parameters"""
        try:
    ...
    def _load_and_prepare_video(
        self, video_path: Path, resource_manager: ResourceManager
    ) -> Optional[VideoFileClip]:
    ...
    def _apply_visual_effects(
        ...
        try:
            ...
            if analysis.visual_cues:
                try:
                    enhanced_clip = self.effects.add_visual_cues(
                        video_clip, analysis.visual_cues
                    )
            ...
            if hasattr(analysis, "focus_points") and analysis.focus_points:
                try:
                    pan_zoom_clip = self.effects.apply_dynamic_pan_zoom(
                        video_clip, analysis.focus_points
                    )
            ...
            if analysis.speed_effects:
                try:
                    speed_clip = self.effects.apply_speed_effects(
                        video_clip, analysis.speed_effects
                    )
            ...
            try:
                ...
                graded_clip = self.advanced_enhancer.apply_adaptive_color_grading(
                    video_clip
                )
            ...
            if quality_profile == "maximum":
                try:
                    ...
                    self.logger.warning(
                        f"Failed to apply advanced enhancement filters: {e}"
                    )
    ...
    def _add_text_overlays(
        self, video_clip: VideoFileClip, analysis: VideoAnalysis
    ) -> VideoFileClip:
        """Add text overlays with error handling"""
        try:
            if analysis.text_overlays:
                overlay_clip = self.text_processor.add_text_overlays(
                    video_clip, analysis.text_overlays
                )
                return overlay_clip
            return video_clip
    ...
    def _process_audio(
        ...
        try:
            ...
            if final_audio:
                ...
                self.logger.warning(
                    "Audio processing returned None, keeping original audio"
                )
                return video_clip
        ...
        try:
            ...
            try:
                if current_aspect > target_aspect:
                    ...
                    clip = MoviePyCompat.crop(
                        clip, x1=x_offset, x2=x_offset + new_width
                    )
                    ...
                    clip = MoviePyCompat.crop(
                        clip, y1=y_offset, y2=y_offset + new_height
                    )
                ...
                if hasattr(clip, "subclip"):
                    clip = MoviePyCompat.subclip(
                        clip, 0, min(clip.duration, 60)
                    )  # Limit duration
    ...
    def _write_video(self, clip: VideoFileClip, output_path: Path):
        ...
        try:
            ...
            speed_optimization = getattr(
                self.config.video, "enable_speed_optimization", True
            )
            speed_level = getattr(
                self.config.video, "speed_optimization_level", "aggressive"
            ).lower()

            # Use speed optimizer for optimal settings if enabled
            if speed_optimization:
                optimal_settings = self.speed_optimizer.get_optimal_settings(
                    quality_profile, speed_level
                )
                ...
                self.logger.info(
                    f"Using speed-optimized settings: {codec}/{preset}, CRF={crf}, {optimal_threads} threads"
                )
            else:
            ...
            if "nvenc" in codec:
                try:
                    ...
                    self.logger.warning(
                        "NVENC requested but GPU not available, falling back to CPU"
                    )
                    codec = "libx264"
                    preset = "ultrafast"
            ...
            self.logger.info(
                f"Starting video encoding with profile: {quality_profile}, speed: {speed_level}"
            )
