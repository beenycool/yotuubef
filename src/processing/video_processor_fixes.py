...
class MoviePyCompat:
        ...
        try:
            if hasattr(clip, "with_position"):
                ...
                if hasattr(clip, "at"):
                    ...
                    logging.warning(
                        f"No position method found for {type(clip).__name__}"
                    )
                    return clip
        except Exception as e:
        ...
        try:
            ...
            try:
                ...
                if hasattr(clip, "fl"):
                    ...
                    logging.debug(
                        f"No fl method found for {type(clip).__name__}, trying manual application"
                    )
                    ...
                    if hasattr(clip, "make_frame"):
                        try:
                            ...
                            new_clip = clip.copy()
                            new_clip.make_frame = lambda t: effect_func(
                                clip.make_frame, t
                            )
                            return new_clip
                        except:
                    ...
                    logging.debug(
                        f"Returning original {type(clip).__name__} clip - effect not applicable"
                    )
                    return clip
    ...
    def create_text_clip(text, **kwargs):
        ...
        if "font" in clean_kwargs:
            ...
            if not Path(font_path).exists() and font_path != "Arial":
                logging.warning(
                    f"Font file not found: {font_path}, falling back to Arial"
                )
    ...
    def crop(clip, x1=None, y1=None, x2=None, y2=None, width=None, height=None):
        ...
        try:
            ...
            # Fall back to old API
                ...
                return MoviePyCompat.apply_effect(clip, crop_frame).resize(
                    (new_w, new_h)
                )
        except Exception as e:
    ...
    def with_audio(video_clip, audio_clip):
        ...
        if not hasattr(video_clip, "with_audio") and not hasattr(
            video_clip, "set_audio"
        ):
            raise AttributeError(
                "Neither 'with_audio' nor 'set_audio' method found on video clip"
            )
            ...
            try:
                ...
                if hasattr(video_clip, "set_audio"):
                    try:
                        ...
                        logger.warning(
                            f"Failed to set audio on video clip with both methods: {e}, {e2}"
                        )
                        return video_clip
                else:
        ...
        try:
            if hasattr(clip, "crossfadein"):
                ...
                if hasattr(clip, "with_opacity"):
                    ...
                    logging.warning(
                        f"No crossfadein method available for {type(clip).__name__}"
                    )
                    return clip
        except Exception as e:
        ...
        try:
            if hasattr(clip, "crossfadeout"):
                ...
                if hasattr(clip, "with_opacity"):
                    ...
                    logging.warning(
                        f"No crossfadeout method available for {type(clip).__name__}"
                    )
                    return clip
        except Exception as e:
...
def ensure_shorts_format(
    clip: VideoFileClip, target_duration: float = 60.0
) -> VideoFileClip:
    """
    ...
    if clip.duration > target_duration:
        logger.info(
            f"Trimming video from {clip.duration:.1f}s to {target_duration}s for Shorts"
        )
...
class VideoProcessorFixes:
    ...
    def crop(clip, x1=None, y1=None, x2=None, y2=None, width=None, height=None):
        ...
        return MoviePyCompat.crop(
            clip, x1=x1, y1=y1, x2=x2, y2=y2, width=width, height=height
        )
