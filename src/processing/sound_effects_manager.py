...
class SoundEffectsManager:
    ...
    def refresh_sound_effects_cache(self) -> None:
        ...
        try:
            if not self.sound_effects_dir.exists():
                self.logger.warning(
                    f"Sound effects directory not found: {self.sound_effects_dir}"
                )
                return
            ...
            for category, info in self.category_mapping.items():
                ...
                self.logger.debug(
                    f"Found {len(category_files)} effects in {category} category"
                )
            ...
            self.logger.info(
                f"Sound effects cache refreshed: {total_effects} effects across {len(self._sound_cache)} categories"
            )
    ...
    def find_sound_effect(
        self, effect_name: str, preferred_category: Optional[str] = None
    ) -> Optional[Path]:
        """
        ...
        if preferred_category and preferred_category in self._sound_cache:
            direct_match = self._find_in_category(
                effect_name, preferred_category, exact=True
            )
            if direct_match:
                self.logger.debug(
                    f"Found direct match for '{effect_name}' in {preferred_category}: {direct_match}"
                )
                return direct_match
        ...
        for category in self._sound_cache:
            ...
            if direct_match:
                self.logger.debug(
                    f"Found direct match for '{effect_name}' in {category}: {direct_match}"
                )
                return direct_match

        # Strategy 3: Alias match in preferred category
        if preferred_category and preferred_category in self._sound_cache:
            alias_match = self._find_in_category(
                effect_name, preferred_category, exact=False
            )
            if alias_match:
                self.logger.debug(
                    f"Found alias match for '{effect_name}' in {preferred_category}: {alias_match}"
                )
                return alias_match
        ...
        if suggested_category and suggested_category != preferred_category:
            category_match = self._find_in_category(
                effect_name, suggested_category, exact=False
            )
            if category_match:
                self.logger.debug(
                    f"Found category match for '{effect_name}' in {suggested_category}: {category_match}"
                )
        ...
        for category in self._sound_cache:
            ...
            if partial_match:
                self.logger.debug(
                    f"Found partial match for '{effect_name}' in {category}: {partial_match}"
                )
        ...
        if fallback_match:
            self.logger.info(
                f"Using fallback effect for '{effect_name}': {fallback_match}"
            )
    ...
    def _find_in_category(
        self, effect_name: str, category: str, exact: bool = True
    ) -> Optional[Path]:
        ...
        for sound_effect in self._sound_cache[category]:
            if exact:
                ...
                if (
                    effect_name == sound_effect.name
                    or effect_name in sound_effect.aliases
                    or any(alias in sound_effect.name for alias in [effect_name])
                ):
                    return sound_effect.path
    ...
    def _find_partial_match(self, effect_name: str, category: str) -> Optional[Path]:
        ...
        for sound_effect in self._sound_cache[category]:
            if (
                effect_name in sound_effect.name
                or sound_effect.name in effect_name
                or any(
                    effect_name in alias or alias in effect_name
                    for alias in sound_effect.aliases
                )
            ):
                return sound_effect.path
    ...
    def _find_fallback_effect(self, effect_name: str) -> Optional[Path]:
        ...
        if suggested_category and suggested_category in self.category_mapping:
            ...
            for fallback_category in fallback_categories:
                if (
                    fallback_category in self._sound_cache
                    and self._sound_cache[fallback_category]
                ):
        ...
        try:
            ...
            with AudioFileClip(str(file_path)) as audio_clip:
                ...
                if audio_clip.duration > 10:  # Warn about long effects
                    return (
                        True,
                        f"Warning: Audio file is quite long ({audio_clip.duration:.1f}s)",
                    )
            ...
            with AudioFileClip(str(file_path)) as audio_clip:
                ...
                if audio_clip.duration > 0:
                    try:
                        ...
                        sample_audio = MoviePyCompat.subclip(
                            audio_clip, 0, min(1.0, audio_clip.duration)
                        )
    ...
    def get_cache_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "category_breakdown": {
                cat: len(effects) for cat, effects in self._sound_cache.items()
            },
            "sound_effects_dir": str(self.sound_effects_dir),
            "cache_populated": total_effects > 0,
        }

    # Event-triggered SFX methods for Improvement 5

    def get_sound_for_event(self, event_type: str) -> Optional[Path]:
        """
        Get the appropriate sound effect for a specific video event.

        Args:
            event_type: Type of event triggering the sound
                - "broll_appear": When B-roll image pops up (whoosh)
                - "hook_start": Opening hook (sub-bass boom)
                - "text_pop": Text overlay appears (click/pop)
                - "transition": Scene transition (whoosh/swoosh)
                - "emphasis": Dramatic emphasis (impact/boom)

        Returns:
            Path to sound effect file, or None
        """
        event_sound_map = {
            "broll_appear": ["whoosh", "swoosh", "transition"],
            "hook_start": ["boom", "dramatic", "impact"],
            "text_pop": ["click", "pop", "snap"],
            "transition": ["whoosh", "swoosh", "wind"],
            "emphasis": ["impact", "boom", "dramatic"],
        }

        sound_options = event_sound_map.get(event_type, [event_type])

        # Try each sound option
        for sound_name in sound_options:
            sound_path = self.find_sound_effect(sound_name)
            if sound_path:
                self.logger.debug(f"Event '{event_type}' -> sound '{sound_name}'")
                return sound_path

        # Fallback to any available sound
        self.logger.warning(
            f"No specific sound for event '{event_type}', using fallback"
        )
        return self._find_fallback_effect(event_type)

    def get_whoosh_sound(self) -> Optional[Path]:
        """Get a whoosh/transition sound effect"""
        return self.get_sound_for_event("broll_appear")

    def get_boom_sound(self) -> Optional[Path]:
        """Get a boom/dramatic sound effect"""
        return self.get_sound_for_event("hook_start")

    def get_pop_sound(self) -> Optional[Path]:
        """Get a pop/click sound effect"""
        return self.get_sound_for_event("text_pop")
