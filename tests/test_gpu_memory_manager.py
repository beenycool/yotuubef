...
def test_init_with_cuda(with_cuda):
    ...
    assert manager.vram_limit_mb == 4096  # int(8 * 1024 * 0.5)
...
def test_optimize_model_for_inference_cpu_no_params(with_torch_no_cuda):
    manager = GPUMemoryManager()
    mock_model = MagicMock()
    mock_model.parameters = lambda: iter([])
...
def test_optimize_model_for_inference_cuda_no_params(with_cuda):
    manager = GPUMemoryManager()
    mock_model = MagicMock()
    mock_model.parameters = lambda: iter([])
...
def test_get_memory_manager():
    original_manager = gmm._memory_manager
    try:
        ...
        gmm._memory_manager = original_manager
