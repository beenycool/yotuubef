#!/usr/bin/env python3
"""
Demo script for long-form video generation
Creates a sample video structure to demonstrate the functionality
"""

import json
import sys
from pathlib import Path

# Add the src directory to Python path
sys.path.insert(0, str(Path(__file__).parent))

from src.models import (
    VideoFormat, ContentStructureType, NicheCategory, 
    ContentSection, NicheTopicConfig, LongFormVideoStructure,
    LongFormVideoAnalysis, NarrativeSegment, EmotionType, PacingType
)

def create_sample_video_structure():
    """Create a sample long-form video structure"""
    
    # Create niche configuration
    niche_config = NicheTopicConfig(
        category=NicheCategory.TECHNOLOGY,
        target_audience="beginner programmers and developers",
        expertise_level="beginner",
        tone="informative",
        keywords=["python", "programming", "tutorial", "beginners", "coding"]
    )
    
    # Create intro section
    intro_section = ContentSection(
        section_type=ContentStructureType.INTRO,
        title="Introduction to Python Programming",
        content="Welcome to this comprehensive Python tutorial! Python is one of the most popular programming languages in the world, known for its simplicity and readability. In this video, we'll take you from complete beginner to having a solid foundation in Python programming.",
        duration_seconds=45.0,
        key_points=[
            "Python is beginner-friendly",
            "Used by major companies like Google and Netflix",
            "Great for web development, data science, and automation",
            "What you'll learn in this tutorial"
        ],
        visual_suggestions=[
            "Python logo animation",
            "Code examples scrolling",
            "Companies using Python",
            "Tutorial overview graphics"
        ]
    )
    
    # Create body sections
    body_sections = [
        ContentSection(
            section_type=ContentStructureType.BODY,
            title="Python Basics and Syntax",
            content="Let's start with the fundamental concepts of Python. We'll cover variables, data types, and basic syntax. Python's syntax is designed to be readable and intuitive, making it perfect for beginners. We'll write our first Python program and understand how Python interprets our code.",
            duration_seconds=120.0,
            key_points=[
                "Variables and naming conventions",
                "Basic data types (strings, integers, floats)",
                "Python syntax rules",
                "Writing your first Python program",
                "Understanding Python's indentation"
            ],
            visual_suggestions=[
                "Code editor with syntax highlighting",
                "Variable assignment examples",
                "Data type demonstrations",
                "Live coding session"
            ]
        ),
        ContentSection(
            section_type=ContentStructureType.BODY,
            title="Control Flow and Functions",
            content="Now that we understand the basics, let's learn about control flow statements like if-else conditions and loops. We'll also explore functions, which are reusable blocks of code that make our programs more organized and efficient. Functions are essential for writing clean, maintainable code.",
            duration_seconds=150.0,
            key_points=[
                "If-else conditional statements",
                "For and while loops",
                "Creating and calling functions",
                "Function parameters and return values",
                "Best practices for function design"
            ],
            visual_suggestions=[
                "Flowchart diagrams",
                "Loop iteration examples",
                "Function definition and calling",
                "Code organization examples"
            ]
        ),
        ContentSection(
            section_type=ContentStructureType.BODY,
            title="Working with Data Structures",
            content="Python provides several built-in data structures that help us organize and manipulate data efficiently. We'll explore lists, dictionaries, tuples, and sets. Understanding these data structures is crucial for solving real-world programming problems and building practical applications.",
            duration_seconds=135.0,
            key_points=[
                "Lists: ordered, mutable collections",
                "Dictionaries: key-value pairs",
                "Tuples: immutable sequences",
                "Sets: unique element collections",
                "When to use each data structure"
            ],
            visual_suggestions=[
                "Visual representation of data structures",
                "Interactive examples",
                "Real-world use cases",
                "Performance comparisons"
            ]
        )
    ]
    
    # Create conclusion section
    conclusion_section = ContentSection(
        section_type=ContentStructureType.CONCLUSION,
        title="Next Steps and Resources",
        content="Congratulations! You've learned the fundamental concepts of Python programming. You now have a solid foundation to build upon. The key to mastering Python is practice and building real projects. I recommend starting with small projects and gradually increasing complexity as you gain confidence.",
        duration_seconds=60.0,
        key_points=[
            "Practice regularly with coding challenges",
            "Build small projects to apply your knowledge",
            "Join Python communities and forums",
            "Explore Python libraries and frameworks",
            "Consider advanced topics like web development or data science"
        ],
        visual_suggestions=[
            "Project ideas showcase",
            "Resource recommendations",
            "Community links",
            "Subscribe and follow prompts"
        ]
    )
    
    # Create complete video structure
    video_structure = LongFormVideoStructure(
        title="Complete Python Tutorial for Beginners - From Zero to Hero",
        description="Learn Python programming from scratch in this comprehensive tutorial. Perfect for beginners who want to master Python basics including variables, functions, data structures, and more. No prior programming experience required!",
        niche_config=niche_config,
        intro_section=intro_section,
        body_sections=body_sections,
        conclusion_section=conclusion_section,
        total_duration_seconds=510.0,  # 8.5 minutes
        hashtags=[
            "#python", "#programming", "#tutorial", "#beginners", 
            "#coding", "#learnpython", "#pythonbasics", "#development"
        ]
    )
    
    return video_structure

def create_sample_analysis():
    """Create a sample long-form video analysis"""
    
    video_structure = create_sample_video_structure()
    
    # Create sample narration segments
    narration_segments = [
        NarrativeSegment(
            text="Welcome to this comprehensive Python tutorial!",
            time_seconds=0.0,
            intended_duration_seconds=3.0,
            emotion=EmotionType.EXCITED,
            pacing=PacingType.NORMAL
        ),
        NarrativeSegment(
            text="Python is one of the most popular programming languages in the world.",
            time_seconds=3.0,
            intended_duration_seconds=4.0,
            emotion=EmotionType.NEUTRAL,
            pacing=PacingType.NORMAL
        ),
        NarrativeSegment(
            text="Let's start with the fundamental concepts of Python.",
            time_seconds=45.0,
            intended_duration_seconds=3.0,
            emotion=EmotionType.EXCITED,
            pacing=PacingType.NORMAL
        )
    ]
    
    # Create sample analysis
    analysis = LongFormVideoAnalysis(
        video_format=VideoFormat.LONG_FORM,
        video_structure=video_structure,
        detailed_narration=narration_segments,
        section_transitions=[
            "Now let's dive into the fundamentals...",
            "Next, we'll explore control flow...",
            "Finally, let's look at data structures...",
            "To wrap things up..."
        ],
        visual_cues=[],
        text_overlays=[],
        target_audience_analysis={
            "primary_audience": "beginner programmers and developers",
            "expertise_level": "beginner",
            "learning_style": "visual and practical",
            "motivation": "career advancement, skill building"
        },
        engagement_hooks=[
            "You'll be able to write Python programs by the end of this video",
            "Coming up: we'll build a real project together",
            "This technique will save you hours of debugging",
            "The one thing every Python developer needs to know"
        ]
    )
    
    return analysis

def main():
    """Create and display sample long-form video content"""
    
    print("🎬 Long-Form Video Generation Demo")
    print("=" * 50)
    
    # Create sample video structure
    print("\n📋 Creating sample video structure...")
    video_structure = create_sample_video_structure()
    
    print(f"✅ Video Title: {video_structure.title}")
    print(f"📊 Niche: {video_structure.niche_config.category}")
    print(f"👥 Target Audience: {video_structure.niche_config.target_audience}")
    print(f"⏱️ Duration: {video_structure.total_duration_seconds/60:.1f} minutes")
    print(f"📝 Sections: {video_structure.get_total_sections()}")
    
    # Display section breakdown
    print(f"\n📑 Section Breakdown:")
    print(f"  📌 Intro: {video_structure.intro_section.title} ({video_structure.intro_section.duration_seconds}s)")
    
    for i, section in enumerate(video_structure.body_sections, 1):
        print(f"  📄 Body {i}: {section.title} ({section.duration_seconds}s)")
    
    print(f"  🎯 Conclusion: {video_structure.conclusion_section.title} ({video_structure.conclusion_section.duration_seconds}s)")
    
    # Create sample analysis
    print(f"\n🔍 Creating sample analysis...")
    analysis = create_sample_analysis()
    
    print(f"✅ Analysis complete!")
    print(f"💬 Narration segments: {len(analysis.detailed_narration)}")
    print(f"🔄 Section transitions: {len(analysis.section_transitions)}")
    print(f"🎣 Engagement hooks: {len(analysis.engagement_hooks)}")
    
    # Save to file
    output_dir = Path("data/examples")
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # Save video structure
    structure_file = output_dir / "sample_video_structure.json"
    with open(structure_file, 'w') as f:
        json.dump(video_structure.model_dump(), f, indent=2)
    
    # Save analysis
    analysis_file = output_dir / "sample_video_analysis.json"
    with open(analysis_file, 'w') as f:
        json.dump(analysis.model_dump(), f, indent=2)
    
    print(f"\n💾 Sample files saved:")
    print(f"  📄 Structure: {structure_file}")
    print(f"  📊 Analysis: {analysis_file}")
    
    # Display usage instructions
    print(f"\n🚀 To generate a similar video, run:")
    print(f'python main.py longform "Complete Python Tutorial for Beginners" --niche technology --audience "beginner programmers"')
    
    print(f"\n✨ Long-form video generation is ready to use!")

if __name__ == "__main__":
    main()